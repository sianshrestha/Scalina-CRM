...
export const TeamManagement: React.FC = () => {
    ...
    // STRATEGIC UPDATE: Added 'Marketer'
    const AVAILABLE_ROLES = ['Admin', 'Shoot', 'Edit', 'Script', 'Marketer'];
    ...
    const getDisplayName = (member: TeamMember) => {
        return  `${member.firstName || ''} ${member.lastName || ''}`.trim() || member.name || 'Unnamed';
    };

    const renderTextInput = (label: string, field: keyof TeamMemberForm, options: { type?: string; required?: boolean; placeholder?: string } = {}) => (
        <div>
    ...
    const getRoleColor = (roleName: string) => {
        ...
        if (roleName === 'Marketer') return 'bg-emerald-50 text-emerald-600 border-emerald-200'; // New Marketer Color
        ...
        <div className="p-6 bg-gray-50 min-h-screen flex flex-col gap-6">
            ...
            <div className="flex justify-between items-end shrink-0">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">Manage agency staff, marketers, and system access roles.</p>
                </div>
                <button onClick={() => { resetForm(); setShowFormModal(true); }} className="bg-blue-600 text-white px-5 py-2 rounded-lg shadow hover:bg-blue-700 font-medium transition flex items-center gap-2">
            ...
            <div className="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden flex-1">
                <table className="min-w-full w-full">
                    ...
                    {teamMembers.length === 0 ? (
                        <tr><td colSpan={3} className="text-center py-10 text-gray-400 text-sm">No team members found.</td></tr>
                    ) : (
                                ...
                                <tr key={member.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right">
                                        ...
                                        <button
                                            ...
                                            className="text-[10px] font-bold text-gray-500 hover:text-white hover:bg-gray-500 border border-transparent hover:border-gray-600 px-3 py-1.5 rounded transition mr-2"
                                        >
                                            View
                                        </button>
                                        <button
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white p-8 rounded-2xl w-[600px] max-h-[90vh] shadow-2xl flex flex-col">
                        <div className="flex justify-between items-start mb-6 shrink-0">
                            <div>
                                <h3 className="text-2xl font-bold text-gray-800">{getDisplayName(viewingMember)}</h3>
                                <span className={`inline-block mt-2 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest border ${getRoleColor(viewingMember.role)}`}>
                                    {viewingMember.role}
                                </span>
                            </div>
                            <button onClick={() => setViewingMember(null)} className="text-gray-400 hover:text-gray-600 text-xl font-bold">✕</button>
                        </div>

                        <div className="overflow-y-auto pr-2 space-y-6">
                            <div>
                                <h4 className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-3 border-b border-gray-100 pb-1">Personal Details</h4>
                                <div className="grid grid-cols-2 gap-4 text-sm text-gray-700">
                                    <div><span className="block text-[10px] font-bold text-gray-400 uppercase">Email</span>{viewingMember.personalEmail || '—'}</div>
                                    <div><span className="block text-[10px] font-bold text-gray-400 uppercase">Phone</span>{viewingMember.phoneNumber || '—'}</div>
                                    <div><span className="block text-[10px] font-bold text-gray-400 uppercase">DOB</span>{viewingMember.dob || '—'}</div>
                                    <div><span className="block text-[10px] font-bold text-gray-400 uppercase">Nationality</span>{viewingMember.nationality || '—'}</div>
                                </div>
                            </div>

                            <div>
                                <h4 className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-3 border-b border-gray-100 pb-1">Residential Address</h4>
                                <div className="grid grid-cols-2 gap-4 text-sm text-gray-700">
                                    <div><span className="block text-[10px] font-bold text-gray-400 uppercase">Street</span>{viewingMember.streetAddress || '—'}</div>
                                    <div><span className="block text-[10px] font-bold text-gray-400 uppercase">State & Postcode</span>{viewingMember.residentialState || '—'} {viewingMember.postcode || ''}</div>
                                    <div><span className="block text-[10px] font-bold text-gray-400 uppercase">Country</span>{viewingMember.residentialCountry || '—'}</div>
                                </div>
                            </div>

                            <div>
                                <h4 className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-3 border-b border-gray-100 pb-1">Bank Details</h4>
                                <div className="grid grid-cols-2 gap-4 text-sm text-gray-700">
                                    <div><span className="block text-[10px] font-bold text-gray-400 uppercase">Bank Name</span>{viewingMember.bankName || '—'}</div>
                                    <div><span className="block text-[10px] font-bold text-gray-400 uppercase">Account Name</span>{viewingMember.accountName || '—'}</div>
                                    <div><span className="block text-[10px] font-bold text-gray-400 uppercase">Account #</span>{viewingMember.accountNumber || '—'}</div>
                                    {viewingMember.bankCountry === 'Australia' && <div><span className="block text-[10px] font-bold text-gray-400 uppercase">BSB</span>{viewingMember.bsb || '—'}</div>}
                                </div>
                            </div>

                            <div>
                                <h4 className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-3 border-b border-gray-100 pb-1">Emergency Contact</h4>
                                <div className="grid grid-cols-2 gap-4 text-sm text-gray-700">
                                    <div><span className="block text-[10px] font-bold text-gray-400 uppercase">Name</span>{viewingMember.emergencyContactName || '—'}</div>
                                    <div><span className="block text-[10px] font-bold text-gray-400 uppercase">Number</span>{viewingMember.emergencyContactNumber || '—'}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-8 rounded-2xl w-[760px] max-h-[90vh] shadow-2xl flex flex-col">
                        ...
                        <form onSubmit={handleSaveMember} className="space-y-6 overflow-y-auto pr-1">
                            ...
                            <div className="flex gap-3 pt-4 mt-2 border-t border-gray-100 sticky bottom-0 bg-white">
                                <button type="button" onClick={closeFormModal} className="flex-1 bg-gray-100 p-3 rounded-xl font-bold text-gray-600 hover:bg-gray-200 transition">Cancel</button>
